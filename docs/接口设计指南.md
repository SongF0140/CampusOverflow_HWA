...
> 状态：v1（2026-09-07）；范围注（2026-09-20）：第 3 节接口清单基于 spec v1 展开，实际路径与范围以 spec v3 / plan v3 / docs/后端架构说明.md 为准；第 4 节内部 API 与第 5 节 Agent API 为第二阶段内容（spec v3 / Q-08）
> 适用读者：三端开发成员（前端 / 业务后端 / Agent），新手向
> 上游文档：[后端架构（按端拆分）](./后端架构/README.md)（三端接口权威清单）、[plan.md](../specs/plan.md) 第 5 节（内部 API 草案）、[开发协作流程文档](./开发协作流程文档.md) 第一 / 二步（清单 → 流程图 → 冻结）
...
3. **字段一律蛇形（snake_case，2026-09-20 定案）**：`created_at`、`course_id`、`page_size`——与 Python / SQLAlchemy 主流一致，已实现接口（T-02）即为该风格；TypeScript 侧（前端 / Agent 代码内部）仍用 camelCase，仅在接口边界按蛇形解析。
4. **列表接口必带分页**：请求 `?page=1&page_size=20`，响应 data 里带 `total`。需要排序就加 `sort` 参数（如 `sort=latest|hot`），需要筛选就加专用参数（如 `course_id`、`tag_id`、`unresolved=true`）。
...
| 方法 | 路径 | 用途 | 备注 |
| ---- | ---- | ---- | ---- |
| ★ | GET /api/courses | 课程列表（可按学期 / 关键词筛） | 登录用户可看 |
...
| | GET /api/courses/{id}/members | 成员列表 | 教师 / 管理员 |
| | POST /api/courses/{id}/members | 学生加入课程 | |
| | DELETE /api/courses/{id}/members/me | 学生退出课程 | |

### 3.3 问题（T-04 / US-03、US-09）

| 方法 | 路径 | 用途 | 备注 |
| ---- | ---- | ---- | ---- |
| ★ | POST /api/questions | 发布问题 | body 为 Markdown，入库前 XSS 清洗；带 `courseId`、`tagIds` |
| ★ | GET /api/questions/{id} | 问题详情 | 浏览数 +1；登录用户可看 |
| ★ | GET /api/questions | 问题列表（分页 + `courseId`/`tagId`/`sort=latest|hot`/`unresolved` 筛选） | 登录用户可看；筛选覆盖 US-09 |
...
> 实现在 FastAPI 的 `/internal/agent/*` 前缀下（main.py 预留前缀，二期注册路由）；**白名单之外一概 403**。每条请求带 `X-Service-Token` 与 `X-Trace-Id`。签名细节以 [plan.md](../specs/plan.md) 第 5 节为准，下表是执行清单。
...
> **与 spec 对齐的说明**：`answer-assist` 与 `docs/draft` 曾在旧版路径规划中有路径，但 spec.md v1 的 US-11~19 中**没有对应故事**（spec 的 US-14 实为"处理审核工单"，不是草稿生成）。按宪法优先级，第一期不实现这两条，列入第二阶段候选；若要纳入，先走 `/speckit.specify` 补故事再重跑 analyze。
...
全过 → 把条目回填 [plan.md](../specs/plan.md) 第 5 节 + 对应端接口文档（[docs/后端架构/](./后端架构/)），PR 走 SM 审查。

...
> 状态：v1（2026-09-06）；范围注（2026-09-20）：本期只做业务后端——第 3~4 节中 Agent 服务与前端的安装/启动步骤为第二阶段内容，日常任务仅启动后端即可（spec v3 / Q-08）
...
上下文：specs/plan.md 第 5 节接口草案、docs/后端架构/Agent端预留文档.md
约束：遵守硬性约束 1/5；内部接口在 backend/app/main.py 的 /internal/agent/* 前缀下实现，同时改 agent/src/
...
- 改动是否越界（T-02 类任务只应动 `backend/app/modules/identity` + tests）；
...
git add campus-overflow-ai/backend/app/modules/identity campus-overflow-ai/backend/tests specs/tasks.md
